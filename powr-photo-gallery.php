<?php
    /**
     * @package POWr Photo Gallery
     * @version 1.3
     */
    /*
    Plugin Name: POWr Photo Gallery
    Plugin URI: http://www.powr.io
    Description: A Pinterest-style gallery for unlimited images!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.3
    ...
    */
    ...
    class Powr_Photo_Gallery extends WP_Widget{
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_photo_gallery',
                             __( 'POWr Photo Gallery' ),
                             array( 'description' => __( 'Photo Gallery by POWr.io') )
        );
      }
      //This prints the div
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-photo-gallery' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ?>
        <p>
          No need to edit here - just click the gears icon on your Photo Gallery.
        </p>
        <p>
    ...
    function register_powr_photo_gallery() {
      register_widget( 'Powr_Photo_Gallery' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_photo_gallery' );
    //Create short codes for adding plugins anywhere:
    function powr_photo_gallery_shortcode( $atts ){
      if(isset($atts['label'])){
        $label = $atts['label'];
      }else{
        $label = '';
      }
      return "<div class='powr-photo-gallery' label='$label'></div>";
    }
    add_shortcode( 'powr-photo-gallery', 'powr_photo_gallery_shortcode' );
